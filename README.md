...
Mac 开始计时后可点击“取消计时”，确认后归零，不生成学习记录；休眠前若手动暂停，唤醒后仍保持暂停。退出应用后重新打开仍按已保存草稿恢复为暂停。

## Mac 指令与时间标记

- `!stop`：结束当前专注，填写活动名称并保存时长；原 `stop!` 不再生效。
- `!sex`：记录一次 SEX 和输入当下的日期时间，不记录持续时间或专注度，不改变当前计时。每次回车单独计一次。
- 空输入回车：开始、暂停或继续计时。

时间标记位于“专注记录 → 时间标记”，支持删除、恢复及彻底删除，不参与专注时长、次数或图表统计；随 JSON 自动去重合并。CSV 仍只导出时长记录，完整备份请使用 JSON。iPhone 主页“时间标记”提供相同标记管理能力，点击“标记 SEX 一次”对应 !sex，点击“结束本次专注”对应 !stop。两端应更新后交换 v5 数据。
