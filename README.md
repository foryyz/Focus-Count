...
Mac 开始计时后可在底部更多菜单点击“取消本次计时”，确认后归零，不生成学习记录；休眠前若手动暂停，唤醒后仍保持暂停。退出应用后重新打开仍按已保存草稿恢复为暂停。
...
专注中持续播放低速波纹，暂停时静止；开启系统“减少动态效果”后不播放循环动画。全屏运行时，鼠标静止约 4 秒后隐藏操作区，移动鼠标或输入指令即可恢复；暂停与错误提示时操作区保持可见。全屏不会自动改变系统勿扰设置。原 !stop、!sex、休眠计时和数据交换功能保持可用。
