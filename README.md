...
- 主界面默认 880×580，可自由缩放（最小 720×520），支持原生全屏及右上角全屏按钮。暖白／深石墨底色，时分突出、秒稍小，颜色随系统外观变化。
...
- 输入框默认收起，点击结束右侧的 … 展开；成功标记后自动收起，Esc 可收起。空标记不保存。未编辑标记时可通过回车开始、暂停或继续。
...
准备页面采用英文鼓励语与 emoji，可选填写活动名称或选择历史活动，开始后显示在时间上方。完成保存后给出短暂鼓励并切换下一句文案。
...
Mac 1.6：结束按钮加停止图标；… 直接展开标记输入框，叉号取消计时并保留确认；移除底部 FULL ATTENTION 标语。数据格式仍为 v5，新自定义标记可被 iPhone 显示和合并。
