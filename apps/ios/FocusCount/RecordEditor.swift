...
struct RecordEditor: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("专注内容") {
                    TextField("活动名称", text: $session.subject)
                    if !store.subjects.isEmpty {
                        Menu("选择已有活动") { ForEach(store.subjects, id: \.self) { name in Button(name) { session.subject = name } } }
                    }
                }
                Section {
                ...
                } header: { Text("专注度") } footer: { Text("S 最高 · D 最低，按这次专注的感受选择。") }
                Section {
                    if completesTimer {
                        LabeledContent("有效专注时间", value: phoneDuration(session.activeSeconds))
                    } else {
            ...
            .navigationTitle(completesTimer ? "完成本次专注" : "专注记录").navigationBarTitleDisplayMode(.inline)
            .toolbar {
