...
struct DataExchangeView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            if let incoming {
                let merged = RecordExchange.merge(local: store.database.sessions, incoming: incoming.sessions)
                GroupBox("导入预览") {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        Text("修改时间较新的版本作为当前记录；时间相同按固定规则选定，两端结果一致。其他版本保留，可恢复。导入前备份双方数据，不替换当前计时。")
            ...
            Text("需要手动选择文件导入；此功能不自动联网同步。两端请都更新到支持 v3 的版本。")
                .font(.caption).foregroundStyle(.secondary)
